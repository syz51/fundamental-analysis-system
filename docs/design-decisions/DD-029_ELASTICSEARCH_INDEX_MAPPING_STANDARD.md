  ...
  "mappings": {
    "properties": {
      ...
      "headline": {
        "type": "text",
        "analyzer": "financial_analyzer"
        // Note: Boost parameter deprecated in ES 8.x - use query-time boosting instead
      },
      "author": {
        "type": "keyword",
        "doc_values": true
...
- "Find Bloomberg articles about M&A events mentioning $AAPL or $GOOGL in 2024"
  ...
  - Hybrid search: `headline` + `text` + `embedding`
    ...
    "analysis": {
      ...
      "filter": {
        ...
        "financial_synonyms": {
          "type": "synonym_graph",
          "synonyms": [
            "EBITDA, Earnings Before Interest Taxes Depreciation Amortization, earnings before interest taxes depreciation amortization",
...
- **Financial synonyms**: Uses `synonym_graph` filter for multi-word synonym expansion (search "EBITDA" matches "Earnings Before Interest Taxes...")
